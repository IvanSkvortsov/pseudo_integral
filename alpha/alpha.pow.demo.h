...
int demo_pow(const char * file)
{
	...
	cout << setw(4) << "i" << setw(24) << "alpA" << setw(24) << "kA" <<
		setw(24) << "powkA[2]" << setw(24) << "kA * kA" <<
		endl;
	U ta, tb, tc, _k;
	for(int la = 1, lb = 2; la < m_alp.la_size(); ++la)
	{
		m_alp.map1A_set_lx( la );
		m_alp.map2AB_set_la( la );
		m_alp.map2AB_set_lb( lb );
		m_alp.map1B_set_lx( lb );
		m_alp.map1powkA_set_lx( la );
		...
		tb *= tb;
		cout << setw(20) << m_alp.map1powkA_pos() << setw(16) << m_alp.map1powkA_size() << endl;
		for(int it = 0, i = m_alp.map1A_pos(); it < m_alp.map1A_size(); ++i, ++it )
		{
			...
			_k = ta + tc + tb;
			_k = sqrt( _k );
			m_alp.map1A_set_ix( it );
			m_alp.mx1kA_set_idx();
			m_alp.map1powkA_set_ix( it );
			m_alp.mx1powkA_set_idx();
			cout << setw(4) << i <<
				//setw(24) << setprecision(16) << scientific << m_alp.M_mx1A( i ) <<
				setw(24) << setprecision(16) << scientific << m_alp.mx1A( it ) <<
				setw(24) << setprecision(16) << scientific << m_alp.mx1kA() <<
				//setw(24) << setprecision(16) << scientific << m_alp.mx1kA( la, it ) <<
				//setw(24) << setprecision(16) << scientific << m_alp.M_mx1kA( i ) <<
				//setw(24) << setprecision(16) << scientific << m_alp.mx1powkA( la, it)[2] <<
				setw(24) << setprecision(16) << scientific << m_alp.mx1powkA( 2 ) <<
				//setw(24) << setprecision(16) << scientific << m_alp.M_mx1kA( i ) * m_alp.M_mx1kA( i );
				setw(24) << setprecision(16) << scientific << m_alp.mx1kA( la, it ) * m_alp.mx1kA( la, it );
			if( m_alp.is_mapping_max() && 0 )
			{
				cout <<
...
int demo_pow_d(const char * file)
{
